...
def get_recommendation(
    ...
    llm_call = session.scalar(select(LlmCall).where(LlmCall.job_id == job_id))
    view = RecommendationView.model_validate(recommendation)
    if llm_call is None:
        return view
    return view.model_copy(
        update={
            "synthesis_provider": llm_call.provider,
            "synthesis_model": llm_call.model,
            "synthesis_status": llm_call.status,
            "synthesis_error": llm_call.error_message,
        }
    )
